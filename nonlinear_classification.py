...
class DataReader(object):
    ...
    def ReadData(self):
        ...
        if train_file.exists():
            # 学习之后还是挺多代码理解困难orz，
            # 除了让其适应实验所需数据集输入，本人能做的好像也只有调参了orz
            data = np.genfromtxt(self.train_file_name,
                                 dtype="str", delimiter=",", skip_header=1)
            self.XTrainRaw = data[:, :-1].copy().astype("float")
            self.YTrainRaw = data[:, -1].copy().reshape(len(data[:, -1]), 1)
        ...
        if test_file.exists():
            data = np.genfromtxt(self.test_file_name,
                                 dtype="str", delimiter=",", skip_header=1)
            self.XTestRaw = data[:, :-1].copy().astype("float")
            self.YTestRaw = data[:, -1].copy().reshape(len(data[:, -1]), 1)
    ...
    def NormalizeY(self, nettype, base=0):
        if nettype == NetType.MultipleClassifier:
...
class NeuralNet(object):
    def __init__(self, hp, model_name):
        ...
        self.wb1 = WeightsBias(
            self.hp.num_input, self.hp.num_hidden, self.hp.init_method, self.hp.eta)
        self.wb1.InitializeWeights(self.subfolder, False)
        self.wb2 = WeightsBias(
            self.hp.num_hidden, self.hp.num_output, self.hp.init_method, self.hp.eta)
            ...
            for iteration in range(max_iteration):
                ...
                if (total_iteration+1) % checkpoint_iteration == 0:
                    need_stop = self.CheckErrorAndLoss(
                        dataReader, batch_x, batch_y, epoch, total_iteration)
                    if need_stop:
                        break
                    # end if
                # end if
    ...
    def CheckErrorAndLoss(self, dataReader, train_x, train_y, epoch, total_iteration):
        ...
        need_stop = self.loss_trace.Add(
            epoch, total_iteration, loss_train, accuracy_train, loss_vld, accuracy_vld)
...
class HyperParameters(object):
    ...
    def toString(self):
        title = str.format("bz:{0},eta:{1},ne:{2}",
                           self.batch_size, self.eta, self.num_hidden)
        return title
...
class WeightsBias(object):
    def __init__(self, n_input, n_output, init_method, eta):
        ...
        self.init_method = init_method
        self.eta = eta
        self.initial_value_filename = str.format(
            "w_{0}_{1}_{2}_init", self.num_input, self.num_output, self.init_method.name)
    ...
    def __CreateNew(self):
        self.W, self.B = WeightsBias.InitialParameters(
            self.num_input, self.num_output, self.init_method)
        self.__SaveInitialValue()

    def __LoadExistingParameters(self):
        file_name = str.format(
            "{0}/{1}.npz", self.folder, self.initial_value_filename)
    ...
    def InitialParameters(num_input, num_output, method):
        if method == InitialMethod.Zero:
            ...
            W = np.random.normal(0, np.sqrt(2/num_output),
...
if __name__ == '__main__':
    dataReader = DataReader("./Dataset/iris.csv", "./Dataset/iris.csv")
    dataReader.ReadData()
    ...
    hp = HyperParameters(n_input, n_hidden, n_output, eta, max_epoch,
                         batch_size, eps, NetType.MultipleClassifier, InitialMethod.Xavier)
    net = NeuralNet(hp, "Bank")
